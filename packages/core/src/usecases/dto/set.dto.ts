...
export const AddSetRequestSchema = v.objectAsync({
  ...
  performedAt: v.optional(
    v.nullable(
      v.pipe(
        v.string(),
        v.isoTimestamp(
          "PerformedAt must be a valid ISO 8601 timestamp string.",
        ),
      ),
    ),
  ),
...
export const SetUpdateRequestSchema = v.objectAsync({
  exerciseId: v.optional(
    v.nullable(
      v.pipe(
        v.string(),
        v.uuid("Exercise ID must be a valid UUID if provided."),
      ),
    ),
  ),
  ...
  performedAt: v.optional(
    v.nullable(
      v.pipe(
        v.string(),
        v.isoTimestamp(
          "PerformedAt must be a valid ISO 8601 timestamp string if provided.",
        ),
      ),
    ),
  ),
