...
import type { UserIdVO, ExerciseIdVO } from '../../../domain/shared/vo/identifier'; // これらも .value を使うので値

// Linterの提案に基づき、スキーマ名を exerciseUsage に修正
const exerciseUsageTable = schemaMap.exerciseUsage; 

export class DrizzleExerciseUsageRepository implements IExerciseUsageRepository {
  constructor(
    private readonly db: DrizzleD1Database<typeof schemaMap>,
  ) {}

  async recordUsage(userId: UserIdVO, exerciseId: ExerciseIdVO, performedDate: Date): Promise<void> {
    ...
    if (!exerciseUsageTable) {
        console.error("exerciseUsageTable (schemaMap.exerciseUsage) is not defined in the schema. Make sure it's exported from schema.ts");
        throw new Error("Schema misconfiguration: exerciseUsageTable not found.");
    }

    try {
      await this.db.insert(exerciseUsageTable)
        .values({
          userId: userId.value,
      ...
      console.error(`Failed to record exercise usage for user ${userId.value}, exercise ${exerciseId.value} on ${lastUsedAtIsoString}:`, error);
