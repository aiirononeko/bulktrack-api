...
export function setupSetDependencies(
  ...
  if (!env.DB) {
    ...
    throw new HTTPException(500, {
      message: "Internal Server Configuration Error for Sets",
    });
  }
  const db = drizzle(env.DB, { schema: tablesSchema });

  const workoutSetRepository = new DrizzleWorkoutSetRepository(
    db,
    tablesSchema,
  );
  const exerciseUsageRepository = new DrizzleExerciseUsageRepository(db);

  const appWorkoutService = new AppWorkoutService(
    workoutSetRepository,
    exerciseUsageRepository,
  );
