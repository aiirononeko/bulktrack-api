...
app.post('/device', async (c) => {
  ...
  if (!command) {
    console.error('ActivateDeviceCommand not found in context. DI middleware might not have run.');
  ...
  try {
    const tokens = await command.execute({ deviceId, platform: platform || undefined });
