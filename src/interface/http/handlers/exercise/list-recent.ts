 ...
 */
export const createListRecentExercisesHandler = (listRecentHandler: ListRecentExercisesHandler) => {
  return async (c: Context<AppEnv>): Promise<Response> => {
    try {
      const payload = c.get('jwtPayload');
      if (!payload || typeof payload.sub !== 'string') {
        throw new HTTPException(401, { message: 'Unauthorized: Missing or invalid token subject' });
      }
      ...
      const offset = offsetParam ? Number.parseInt(offsetParam, 10) : DEFAULT_OFFSET;

      if (typeof locale !== 'string') {
        throw new HTTPException(400, { message: 'Invalid locale parameter' });
      }
      if (Number.isNaN(limit) || limit <= 0) { 
        throw new HTTPException(400, { message: 'Invalid limit parameter, must be a number > 0' });
      }
      if (Number.isNaN(offset) || offset < 0) {
        throw new HTTPException(400, { message: 'Invalid offset parameter, must be a number >= 0' });
      }
      ...
      const exercisesDto: ExerciseDto[] = await listRecentHandler.execute(applicationQuery);
